...
# STEP 5: 统计总结与解释
# ============================================================
def statistical_summary(df_results, slope, intercept, r_value, p_value, rho, save_dir):
    """统计总结与发现解读"""
    print("\n" + "=" * 70)
    ...
    summary_df.to_csv(f'{save_dir}/unified_mechanism_summary.csv', index=False)
    
    # 保存详细数据
    df_results.to_csv(f'{save_dir}/unified_mechanism_detailed.csv', index=False)
...
def main():
    ...
    save_dir = rf"{base_dir}\2.1_figures"
    ...
    # Step 3: 主可视化
    slope, intercept, r_value, p_value, rho = plot_main_curve(df_results, save_dir)
    
    # Step 4: 补充可视化
    plot_supplementary(df_results, save_dir)
    
    # Step 5: 统计总结
    summary = statistical_summary(df_results, slope, intercept, r_value, p_value, rho, save_dir)
