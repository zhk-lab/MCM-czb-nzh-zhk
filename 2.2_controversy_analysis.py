...
def main():
    ...
    # Step 1: 识别所有争议选手
    controversy_df = step1_identify_controversy(fan_df, judge_df, save_dir)
    
    # Step 2-3: 全员反事实模拟
    results_df = step2_3_full_simulation(fan_df, judge_df, controversy_df, save_dir)
    
    # Step 4: Suppression统计分析
    suppression_scores, ranking = step4_suppression_analysis(results_df, save_dir)
    
    # Critical Vote分析（针对指定案例）
    critical_data = critical_vote_analysis(fan_df, judge_df, save_dir)
    ...
    create_step1_radar_chart(controversy_df, save_dir)
    create_step2_bump_chart(results_df, save_dir)
    create_critical_vote_viz(critical_data, save_dir)
    create_suppression_dashboard(results_df, suppression_scores, ranking, save_dir)
