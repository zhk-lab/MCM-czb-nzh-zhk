...
# Make Task 1 model importable from Task 2.1 scripts.
REPO_ROOT = Path(__file__).resolve().parents[2]
TASK1_SRC = REPO_ROOT / "task1" / "src"
sys.path.insert(0, str(TASK1_SRC))

from fan_vote_estimation import (
...
def main():
    ...
    fan_shares_csv = str(repo_root / "task1" / "table" / "fan_vote_shares.csv")
    analysis_csv = str(repo_root / "task1" / "table" / "fan_vote_shares_analysis.csv")
    save_dir = str(repo_root / "task2.1" / "figure")
    os.makedirs(os.path.dirname(fan_shares_csv), exist_ok=True)
    os.makedirs(save_dir, exist_ok=True)
